...
export function selectOutputs(config) {
  const {
    ...
    outputs: { include: includeOutputs, exclude: excludeOutputs },
    tags: { include: includeTags, exclude: excludeTags },
    targets: { browser, installer, os, web },
  } = config;
...
function selectOutputsForCategory(
  outputNames,
  tagNames,
  type,
  definitions,
  ...
  for (const target of targets) {
    ...
    if (!definition)
      throw new Error(`Unable to find definition for target.${type}.${target}`);
...
function mapDefinitions(type, names, definitions) {
  ...
  for (const name of names) {
    ...
    if (!definition)
      throw new Error(`Unable to find definition for ${type}.${name}`);
